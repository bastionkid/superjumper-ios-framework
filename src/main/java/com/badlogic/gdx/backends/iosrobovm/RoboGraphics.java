...
public class RoboGraphics extends AbstractGraphics {

    private static final String tag = "RoboGraphics";
    ...
    GLVersion glVersion;
    GLKView view;
    RoboGLViewController viewController;

    public RoboGraphics(RoboApplication app, IOSApplicationConfiguration config, RoboInput input, boolean useGLES30) {
        this.config = config;
        ...
        view = new GLKView(new CGRect(screenBounds.x, screenBounds.y, screenBounds.width, screenBounds.height), context) {
            @Method(selector = "touchesBegan:withEvent:")
            public void touchesBegan (@Pointer long touches, UIEvent event) {
                RoboGraphics.this.input.onTouch(touches);
            }
            ...
            public void draw (CGRect rect) {
                RoboGraphics.this.draw(this, rect);
            }
    ...
    private class IOSViewDelegate extends NSObject implements GLKViewDelegate, GLKViewControllerDelegate {
        @Override
        public void update (GLKViewController controller) {
            RoboGraphics.this.update(controller);
        }

        @Override
        public void willPause (GLKViewController controller, boolean pause) {
            RoboGraphics.this.willPause(controller, pause);
        }

        @Override
        public void draw (GLKView view, CGRect rect) {
            RoboGraphics.this.draw(view, rect);
        }
    }
    ...
    private static class IOSMonitor extends Monitor {
